...
Wil je dit niet bij elke link opnieuw invoeren, voeg dan een stuk Javascript code in. Meer uitleg hierover vind je in het [Javascript hoofdstuk](Javascript.html).
...
De meeste websites blokkeren toegang via 'iframes'. Dit zorgt dus voor problemen wanneer we rapporten in een iframe plaatsen zoals in Swing gedaan wordt.
Om dit op te lossen kunnen we een links de html-eigenschap meegeven `target="_blank"`. Hiermee worden links automatisch in een nieuw tabblad geopend, waardoor de toegang niet geblokkeerd wordt. 

Het zou erg vervelend zijn dit bij alle links te moeten doen. Zie [](javascript-snippets.qmd)
